...
python -m venv venv
```
Activating environment in the termina:
```
source venv/bin/activate 
```

Install all dependencies dev and productions:
```
pip install -r app/requirements-dev.txt 
```
This important pre-commit install:
```
pre-commit install
pre-commit install --hook-type pre-push 
```

# Auto generate aws lib power tools
...
```bash
instituto-castelo-branco$ sam build --use-container
```
...
```bash
instituto-castelo-branco$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
instituto-castelo-branco$ sam local start-api
instituto-castelo-branco$ curl http://localhost:3000/
```
...
```bash
instituto-castelo-branco$ sam logs -n HelloWorldFunction --stack-name instituto-castelo-branco --tail
```
...
```bash
instituto-castelo-branco$ pip install -r tests/requirements.txt --user
# unit test
instituto-castelo-branco$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
instituto-castelo-branco$ AWS_SAM_STACK_NAME="instituto-castelo-branco" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "instituto-castelo-branco"
```
...
Next, you can use AWS Serverless Application Repository to deploy ready to use Apps that go beyond hello world samples and learn how authors developed their applications: [AWS Serverless Application Repository main page](https://aws.amazon.com/serverless/serverlessrepo/)# back-instituto-castelo-branco
